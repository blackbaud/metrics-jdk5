...
public class VirtualMachineMetrics {
    ...
    public Set<String> deadlockedThreads() {
        final long[] threadIds = threads.findMonitorDeadlockedThreads();
        if (threadIds != null) {
    ...
    public void threadDump(OutputStream out) {
        ...
        final ThreadInfo[] threads = this.threads.getThreadInfo(this.threads.getAllThreadIds());
        ...
        for (int ti = threads.length - 1; ti >= 0; ti--) {
            ...
            if (t.getThreadState() != Thread.State.BLOCKED) {
                ...
                writer.printf("\n    - locked (lock name %s) (lock owner %s) (class %s)",
                        t.getLockName(),
                        t.getLockOwnerName(),
                        t.getClass().getName());
